...
REPO="${REPO:-deosha/aigateway}"
BASE_URL="https://raw.githubusercontent.com/${REPO}/v${SCUTUM_VERSION}"

# ----- argument parsing (when piped via `sh -s -- --flag value`) -------------
while [ $# -gt 0 ]; do
    case "$1" in
        --version) SCUTUM_VERSION="$2"; BASE_URL="https://raw.githubusercontent.com/${REPO}/v${SCUTUM_VERSION}"; shift 2 ;;
        --dir)     INSTALL_DIR="$2"; shift 2 ;;
        --repo)    REPO="$2"; BASE_URL="https://raw.githubusercontent.com/${REPO}/v${SCUTUM_VERSION}"; shift 2 ;;
...
fetch() {
    ...
    if curl -fsSL "$src" -o "$out"; then
        ...
        die "could not fetch $src — is v${SCUTUM_VERSION} a published tag at https://github.com/${REPO}/releases ?"
    fi
}

say "Fetching release artifacts (v${SCUTUM_VERSION})"
fetch docker-compose.yaml          "$BASE_URL/docker-compose.release.yaml"
fetch config/license-public.pem    "$BASE_URL/config/license-public.pem"
fetch config/.env.example          "$BASE_URL/config/.env.release.example"
fetch scutum                       "$BASE_URL/scripts/scutum"
chmod +x scutum
